function jBoxWrapper(jQuery) {
    ...
    var jBox = function jBox(type, options) {
        ...
        this.options = {
            ...
            // Other options
            appendTo: jQuery('#admin'),    // The element your jBox will be appended to. Any other element than jQuery('body') is only useful for fixed positions or when position values are numbers
        ...
        this._create = function () {
            ...
            this.wrapper = jQuery('<div/>', {
                id: this.id,
                'class': 'jBox-wrapper-admin' + (this.type ? ' jBox-' + this.type : '') + (this.options.theme ? ' jBox-' + this.options.theme : '') + (this.options.addClass ? ' ' + this.options.addClass : '')
            }).css({
    ...
    jBox.prototype.setContent = function (content, ignore_positioning) {
        ...
        this.content.children('[data-jbox-content-appended]').appendTo('#mobile').css({ display: 'none' });
            ...
            if (!this.isOpen) {
                ...
                if (this.options.fade) {
                    this.wrapper.stop().animate({ opacity: 1 }, {
                        ...
                        start: function () {
                            this.isOpening = true;
                            this.wrapper.css({ display: 'block', visibility: "visible" });
                        }.bind(this),
                        complete: function () {
                    ...
                    this.wrapper.css({ display: 'block', opacity: 1, visibility: "visible" });
            ...
            if (this.isOpen) {
                ...
                if (fadeDuration) {
                    this.wrapper.stop().animate({ opacity: 0 }, {
                        ...
                        complete: function () {
                            this.wrapper.css({ display: 'none', visibility: "hidden" });
                    ...
                    this.wrapper.css({ display: 'none', opacity: 0, visibility: "hidden" });
                    this._fireEvent('onCloseComplete');
                }
            }
        }.bind(this);
...
function jBoxConfirmWrapper(jBox, jQuery) {
    ...
    new jBox.plugin('request', {
        ...
        _onCreated: function () {
            ...
            this.submitButton = jQuery('<button class="btn bt-okay"/>')
    ...
    new jBox.plugin('Add', {
        ...
        _onCreated: function () {
            ...
            // Add a footer to the jBox container
            this.footer = jQuery('<div class="jBox-Confirm-footer confirm"/>');
        ...
        _onOpen: function () {
            ...
            this.submitButton
                ...
                .on('click.jBox-Confirm' + this.id + ' tap.jBox-Confirm' + this.id, function () {
                    if (this.options.confirm) {
                        this.options.confirm(this.source);
                    }
                    // confirm 함수 실행 후 jBox를 닫습니다.
                    this.close();
                }.bind(this));
        }
...
$(function () {
    ...
    new jBox("Add", {
        ...
        title: '컨테이너등록',
        blockScrollAdjust: ['header'],
        content: $('#container-add').html(),
    });


    //컨테이너 수정 등록 창
    new jBox("Add", {
        attach: '.container-mdf',
        addClass: 'info-add',
        confirmButton: "수정 ",
        cancelButton: '닫기',
        closeOnConfirm: true,
    ...
    //컨테이너 장비 등록 창
    new jBox("Add", {
        attach: '.container-device-add',
        addClass: 'info-add container-device-add',
        confirmButton: "등록 ",
        cancelButton: '닫기',
        closeOnConfirm: true,
        width: '80%',
        ...
        title: '컨테이너 장비 등록',
        blockScrollAdjust: ['header'],
        content: $('#container-device-add').html(),
    });

    //컨테이너 장비 수정 창
    new jBox("Add", {
        attach: '.container-device-mdf',
        addClass: 'info-add container-device-add',
        confirmButton: "수정 ",
        cancelButton: '닫기',
        closeOnConfirm: true,
    ...
    new jBox("Add", {
        ...
        title: '장비이력-사유',
        blockScrollAdjust: ['header'],
        content: $('#device-reason').html(),
    });

    // 이미지보기
    $('.open-image').on('click', function () {
        var imageUrl = $(this).data('url'); // 이미지 URL
        var imageTitle = $(this).data('title'); // 이미지 제목
        ...
        new jBox('Modal', {
            ...
            blockScrollAdjust: ['header'],
            title: imageTitle,
            content: '<img src="' + imageUrl + '" alt="' + imageTitle + '">'
        }).open();
    });
    ...
    // 서비스 입력 창
    $('.service-list-view').on('click', function () {
        if ($(this).hasClass('service-list-view')) {
            openInceptionModal(); // 장비 등록창 열기
        }
    });

    let inceptionLevel = 1;
    let offsetLevel = 0;
    let currentModals = 0;
    ...
    function openInceptionModal() {
        ...
        const modal = new jBox('Add', {
            attach: '.service-list-view',
            addClass: 'inception-modal info-add',
            ...
            content: $('#service-list').html(), // 첫 번째 모달 내용
            onCreated: function () {
                // 이미 이벤트가 바인딩되지 않도록 처리
                if (!this.content.data('eventBound')) {
                    this.content.find('.inception-modal-button').on('click', function () {
                        openServiceModal(); // 버튼 클릭 시 서비스 모달 열기
                    });
                    this.content.data('eventBound', true); // 이벤트 바인딩 표시
                }
            },
            onOpen: function () {
                currentModals++;
    ...
    function openServiceModal() {
        // 두 번째 모달 생성
        const serviceModal = new jBox('Add', {
            width: '80%',
            height: 'auto',
            zIndex: 'auto',
            addClass: 'service-modal info-add',
            draggable: 'title',
            closeButton: 'title',
            confirmButton: "등록",
            cancelButton: '닫기',
            title: '장비 서비스 등록',
            blockScrollAdjust: ['header'],
            content: $('#service-add').html(), // 클론된 HTML을 사용
            onOpen: function () {
                currentModals++;
            },
            onCloseComplete: function () {
            ...
            onConfirm: function () {
                // 모달 내부에서 서비스명 입력 필드의 값 가져오기
                const serviceName = this.content.find('.service-name').val().trim();
    ...
    // 서비스 항목 생성
    function addServiceItem(serviceName) {
        ...
        if ($('.service-list').length === 0) {
            $('body').append('<div class="service-list"></div>');
        }
    ...
    // 서비스 삭제 모달창 
    $('body').on('click', '.service-list .item .delete', function () {
        const itemToDelete = $(this).closest('.item'); // 삭제할 항목을 저장
    ...
    // 서비스 상세 보기 모달창
    $('body').on('click', '.service-list .item p', function () {
        const itemContent = $(this).find('p').text().trim(); // 해당 아이템의 내용 가져오기

        // 서비스상세보기 모달
        const detailModal = new jBox('Modal', {
            addClass: 'service-modal info-add',
            zIndex: 'auto',
            height: 'auto',
            title: `서비스상세`,
            content: $('#service-view').html(),
            cancelButton: '닫기',
            closeButton: 'title',
            draggable: 'title',
            overlay: true,
        ...
        detailModal.open(); // 모달 열기
    });


    // 외부계정등록
    new jBox("Add", {
        attach: '.interwork-add',
        addClass: 'info-add',
        confirmButton: "등록 ",
        cancelButton: '닫기',
        closeOnConfirm: true,
        ...
        title: '외부계정등록',
        blockScrollAdjust: ['header'],
        content: $('#interwork-add').html(),
    });

    // 외부계정등록 수정
    new jBox("Add", {
        attach: '.interwork-mdf',
        addClass: 'info-add',
        confirmButton: "수정",
        cancelButton: '닫기',
        closeOnConfirm: true,
